...
fn next_blizzards(blizzards: &mut [Blizzard], width: usize, height: usize) {
...
fn timed_blizzards(
    ...
    let mut timed: Vec<HashSet<Position>> = Vec::with_capacity(n);
    for _ in 0..n {
        let set: HashSet<Position> =
            HashSet::from_iter(blizzards.iter().map(|&(p, _)| p));
        timed.push(set);
        next_blizzards(&mut blizzards, width, height);
    }
    timed
}
    ...
    while let Some((_, (x, y), t)) = queue.pop() {
        ...
        for (dx, dy) in DIRS {
            ...
            for nt in next_time(t, (x, y), p, timed) {
                queue.push((heuristic(p, nt + t), p, nt));
            }
        }
    }
...
pub fn part_one(input: &str) -> usize {
    ...
    trip(start, 0, target, width, height, &timed) + 1
}
...
pub fn part_two(input: &str) -> usize {
    ...
    let t = trip(a, 0, b, width, height, &timed) + 1;
    let t = trip(c, t, d, width, height, &timed) + 1;
    trip(a, t, b, width, height, &timed) + 1
}
